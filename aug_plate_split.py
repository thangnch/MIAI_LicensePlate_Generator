...
def de_noise(image_plate,kernel_size):
    print("Denoise")
    kernel = np.ones((kernel_size, kernel_size), np.uint8)
    image_plate = cv2.morphologyEx(image_plate, cv2.MORPH_CLOSE, kernel)

    #image_plate = cv2.erode(image_plate, kernel, iterations=1)
    #image_plate = cv2.dilate(image_plate, kernel, iterations=1)
    #image_plate = cv2.medianBlur(image_plate, 3)

    return image_plate

def aug_resize(img):
    max_downsize = 50
    downsize = 1/random.randint(1,max_downsize)
    img = cv2.resize(img,dsize=None, fx=1/downsize, fy=1/downsize)
    img = cv2.resize(img, dsize= (CTC_w,CTC_h))
...
def aug_skew(aug_id, aug_num, org_image, image_org_filepath, image_aug_path, label_org_path, label_aug_path, plate_class):
    ...
    while idx < aug_num:
        ...
        if random.random()> 0.6:
            aug_img = aug_resize(aug_img)

        if random.random() > 0.8:
            aug_img = de_noise(aug_img,denoise_amt)
